...
test('Popup Admin Form Inspector Logic', async (t) => {
    ...
    await t.test(
        'inspects form elements and generates valid form_config JSON matching spec 4.1',
        () => {
            const inputs = [
                new MockInput({
                    id: 'user_login',
            ...
            assert.ok(
                inspection.usernameCandidate,
                'Username candidate must be detected',
            );
            assert.strictEqual(inspection.usernameCandidate.id, 'user_login');

            assert.ok(
                inspection.passwordCandidate,
                'Password candidate must be detected',
            );
            assert.strictEqual(inspection.passwordCandidate.id, 'user_pass');

            assert.ok(
                inspection.captchaCandidate,
                'Captcha candidate must be detected',
            );
            ...
            assert.ok(
                config.username_field.selectors.includes("input[name='email']"),
            );
    ...
    await t.test(
        'handles fallback username and default selectors when inputs are empty or generic',
        () => {
            const inputs = [
                new MockInput({
                    id: 'custom_input',
            ...
            assert.ok(
                inspection.usernameCandidate,
                'Fallback username candidate should be selected from text inputs',
            );
            ...
            const emptyConfig =
                popupModule.generateFormConfigJson(emptyInspection);
            assert.deepStrictEqual(emptyConfig.username_field.selectors, [
                '#username',
                "input[name='username']",
                "input[type='email']",
            ]);
            assert.deepStrictEqual(emptyConfig.password_field.selectors, [
                '#password',
                "input[name='password']",
                "input[type='password']",
            ]);
    ...
    await t.test(
        'filters non-text input types like hidden, submit, button, checkbox',
        () => {
            const inputs = [
                new MockInput({
                    id: 'csrf_token',
            ...
            const inspection = popupModule.analyzePageInputs(inputs);
            assert.strictEqual(
                inspection.usernameCandidate.id,
                'real_username',
                'Must not select hidden or checkbox as username',
            );
            assert.strictEqual(
                inspection.passwordCandidate.id,
                'real_password',
            );
        },
    );
    ...
    await t.test(
        'initPopup attaches events and handles DOM inspection and copy flow with sanitization',
        async () => {
            ...
            try {
                ...
                assert.strictEqual(
                    elements['active-tab-domain'].textContent,
                    'sirs.kemkes.go.id (/fo/login)',
                );
                ...
                assert.strictEqual(
                    elements['inspector-results'].classList.contains('hidden'),
                    false,
                );
                assert.ok(
                    elements['json-code-block'].textContent.includes(
                        'txtPassword',
                    ),
                );
                assert.ok(
                    elements['detected-summary'].innerHTML.includes(
                        'Hasil Deteksi:',
                    ),
                );
                ...
                assert.strictEqual(
                    copiedText,
                    elements['json-code-block'].textContent,
                );
                assert.strictEqual(
                    elements['btn-copy-json'].textContent,
                    'Tersalin!',
                );
            } finally {
    ...
    await t.test(
        'initPopup handles chrome.runtime.lastError when executeScript fails',
        () => {
            const originalChrome = globalThis.chrome;
            const originalDoc = globalThis.document;
            ...
            try {
                ...
                assert.strictEqual(
                    elements['btn-inspect-form'].disabled,
                    false,
                );
                assert.strictEqual(
                    elements['btn-inspect-form'].textContent,
    ...
    await t.test(
        'initPopup handles clipboard writeText rejection with error feedback',
        async () => {
            const originalChrome = globalThis.chrome;
            const originalDoc = globalThis.document;
            const desc = Object.getOwnPropertyDescriptor(
            ...
            const elements = {
                'active-tab-domain': { textContent: '' },
                'btn-inspect-form': { listeners: {}, addEventListener() {} },
                'inspector-results': {
                    classList: {
                        remove() {},
                        add() {},
                ...
                'detected-summary': { innerHTML: '' },
                'json-code-block': { textContent: '{"test": 123}' },
                'btn-copy-json': {
                    listeners: {},
                    textContent: 'Salin JSON',
                    addEventListener(event, fn) {
                        this.listeners[event] = fn;
                    },
                },
