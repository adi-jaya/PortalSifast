...
export default function InventarisBarangEdit({ barang, produsen, merk, kategori, jenis }: Props) {
    const { data, setData, put, processing, errors } = useForm({
        kode_barang: barang.kode_barang,
        nama_barang: barang.nama_barang,
        jml_barang: barang.jml_barang?.toString() ?? '',
        kode_produsen: barang.kode_produsen ?? '__none__',
        id_merk: barang.id_merk?.toString() ?? '__none__',
        thn_produksi: barang.thn_produksi?.toString() ?? '',
        isbn: barang.isbn ?? '',
        id_kategori: barang.id_kategori?.toString() ?? '__none__',
        id_jenis: barang.id_jenis?.toString() ?? '__none__',
    });

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(`/inventaris-barang/${barang.kode_barang}`);
    };
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex flex-col gap-4">
                <div className="flex items-start gap-3">
                    ...
                    <Heading
                        title="Edit Barang"
                        description={`Edit data barang: ${barang.nama_barang}`}
                    />
                </div>
                ...
                >
                    ...
                    <div className="flex gap-3 pt-4">
                        ...
                        <Button type="button" variant="outline" asChild>
                            <Link href={`/inventaris-barang/${barang.kode_barang}`}>Batal</Link>
                        </Button>
                    </div>
                </form>
            </div>
