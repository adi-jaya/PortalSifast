...
export default function MutuCategoriesCreate({ scopeOptions, obligationOptions }: Props) {
    const { data, setData, post, processing, errors } = useForm({
        name: '',
        short_name: '',
        scope: scopeOptions[0]?.value ?? 'internal',
        description: '',
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Tambah Kategori Mutu" />

            <div className="flex flex-col gap-4">
                <Heading title="Tambah Kategori Mutu" description="Definisikan kategori indikator mutu." />

                <form
                    onSubmit={(e) => {
                        e.preventDefault();
                        post('/simmutu/categories');
                    }}
