...
export default function Edit({ item }: Props) {
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        { title: 'Edit', href: `/inventaris-jenis/${item.id_jenis}/edit` },
    ];
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex flex-col gap-4">
                <div className="flex items-start gap-3">
                    ...
                    <Heading title="Edit Jenis" description={item.id_jenis} />
                </div>
                <form
                    onSubmit={(e) => {
                        e.preventDefault();
                        patch(`/inventaris-jenis/${item.id_jenis}`);
                    }}
                    className="max-w-xl space-y-6 rounded-xl border bg-card p-6"
                >
                    <div className="grid gap-2">
                        <Label>ID Jenis</Label>
                        <p className="font-mono text-sm text-muted-foreground">{item.id_jenis} (tidak dapat diubah)</p>
                    </div>
                    <div className="grid gap-2">
