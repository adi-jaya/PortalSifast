...
export default function Index({ items, filters }: Props) {
    ...
    const handleSearch = (e: React.FormEvent) => {
        e.preventDefault();
        router.get('/inventaris-kategori', { q: search || undefined }, { preserveState: true });
    };
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex flex-col gap-4">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <Heading title="Master Kategori" description="Master data inventaris SIMRS" />
                    <Button asChild>
                        <Link href="/inventaris-kategori/create">
                        ...
                        <table className="w-full text-left text-sm">
                            ...
                            <tbody>
                                {items.data.length === 0 ? (
                                    ...
                                    items.data.map((item) => (
                                        <tr key={item.id_kategori} className="border-b last:border-0 hover:bg-muted/50">
                                            <td className="px-4 py-3 font-mono">{item.id_kategori}</td>
                                            <td className="px-4 py-3">{item.nama_kategori}</td>
                                            <td className="px-4 py-3">
                                                <div className="flex gap-1">
                                                    <Button variant="ghost" size="icon" asChild>
                                                        <Link href={`/inventaris-kategori/${item.id_kategori}/edit`}>
                                                    ...
                                                    <Button
                                                        ...
                                                        onClick={() => {
                                                            if (confirm('Hapus data ini?')) {
                                                                router.delete(`/inventaris-kategori/${item.id_kategori}`);
                                                            }
                                                        }}
                                                    >
